...
// la imagen de fondo 
  ...
  // caída
    ...
    if(lander.collide(visibleGround)){
      vy -= vy;
    }
  ...
  if(lander.isTouching(enemigosg)  ||  a === 1){
      ...
      textSize(24);
      text("lo siento perdiste",displayWidth-displayWidth+350,displayHeight/4);
      text("preciona R para volverlo a intentar",displayWidth-displayWidth+250,displayHeight/3+100);
  }
...
// mensaje de inicio para el jugador 
  if(a===2){
    ...
    textSize(24);
    text("Al llegar a 40 puntos tendras una habilidad extra",displayWidth/4,displayHeight/4);
    text("Presiona la tecla r para iniciar",displayWidth/3,displayHeight-400);
    enemigosg.destroyEach();
...
function bala(){
  if(frameCount % constante === 0 && score >= 20){
